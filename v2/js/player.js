...
import {drawCluster, drawDices, deletePlayerFromDicesBar, drawDicesNums} from "./draw.js";
...
export class Player {
  ...
  attacking(cluster, target){
    ...
    return {sumPlayer, sumOther}
  }

  successfulAttack(dicesPlayerBefore, target)  {
  ...
  afterSuccessfulAttack(clusters, players, otherId) {
    this.setDices(clusters);
    drawDicesNums(this);
    let other = players.find(p => p.id === otherId)
    other.setDices(clusters);
    if (!clusters.some(c => c.playerId === other.id)) return this.deleteOther(other, players);
    else drawDicesNums(other);
  }

  deleteOther(other, players) {
