...
export class Cluster {
  ...
  path(to) {
    ...
    distances[this.id] = 0;
    const buildPath = (to, predecessors) => {
      let path = [];
      let current = to;
      while (current) {
        path.push(current);
    ...
    const shortestPath = (oldPath, newPath) => {
      let minDicesDiffExisting = Math.min(...oldPath.map((c, i) => this.dices - i - c.dices));
      let minDicesDiffPotential = Math.min(...newPath.map((c, i) => this.dices - i - c.dices));
      return (minDicesDiffPotential > minDicesDiffExisting || (minDicesDiffPotential === minDicesDiffExisting && newPath.length < oldPath.length)) ? newPath : oldPath;
    };
    while (queue.length > 0) {
      ...
      for (let neighbour of neighbours) {
        ...
        if (neighbour.id !== to.id) {
          ...
          let path = buildPath(to, predecessors);
          if (res) res = shortestPath(res, path);
          else res = path
        }
      }
    }
